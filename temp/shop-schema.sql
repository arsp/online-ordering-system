...
ALTER TABLE shop.shop_products
    ...
    REFERENCES shop.shop (id) MATCH SIMPLE
    ON UPDATE NO ACTION
    ON DELETE RESTRICT
    NOT VALID;
...
DROP MATERIALIZED VIEW IF EXISTS shop.order_shop_m_view;

CREATE MATERIALIZED VIEW shop.order_shop_m_view
...
refresh materialized VIEW shop.order_shop_m_view;

DROP function IF EXISTS shop.refresh_order_shop_m_view;

CREATE OR replace function shop.refresh_order_shop_m_view()
returns trigger
AS '
BEGIN
    refresh materialized VIEW shop.order_shop_m_view;
...
DROP trigger IF EXISTS refresh_order_shop_m_view ON shop.shop_products;

CREATE trigger refresh_order_shop_m_view
after INSERT OR UPDATE OR DELETE OR truncate
ON shop.shop_products FOR each statement
EXECUTE PROCEDURE shop.refresh_order_shop_m_view();